...
use us3::service::storage::download::{us3_is_enable, reader_from_env, RangeReader};
...
fn read_aux<P: AsRef<Path>>(path: P) -> std::io::Result<Vec<u8>> {
    ...
    if Path::new((&path).as_ref().as_os_str()).exists() || !us3_enable || r.is_none() {
...
impl<Tree: 'static + MerkleTreeTrait> PrivateReplicaInfo<Tree> {
    pub fn new(replica: PathBuf, comm_r: Commitment, cache_dir: PathBuf) -> Result<Self> {
        ...
        //ensure!(replica.exists(), format!("Sealed replica does not exist,{:?}",&replica));
